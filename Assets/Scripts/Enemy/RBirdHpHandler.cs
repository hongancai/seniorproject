...
public class RBirdHpHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 從GameDB獲取鳥敵人的最大血量
        maxHP = GameDB.bird.enemybased.HP;
        currentHP = maxHP;
        Debug.Log($"初始化完成，最大血量: {maxHP}");
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.T))
        {
            // 每次扣10點血量進行測試
            TakeDamage(10f);
            Debug.Log($"鳥受到10點傷害！當前血量: {currentHP}/{maxHP}");
        }
    ...
    public void TakeDamage(float damage)
    {
        ...
        // 計算實際傷害 (考慮防禦力)
        float actualDamage = Mathf.Max(1, damage - GameDB.bird.enemybased.Def);
        ...
        else if (animator != null && !animator.GetCurrentAnimatorStateInfo(0).IsName("R_dmg"))
        {
    ...
    private void Die()
    {
        ...
        // 通知StormMgr進入死亡狀態，停止移動
        RBirdMgr rbirdManager = GetComponentInParent<RBirdMgr>();
        if (rbirdManager != null)
        {
            rbirdManager.SetDead();
        }
