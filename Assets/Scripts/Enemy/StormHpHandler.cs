...
public class StormHpHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 從GameDB獲取暴風敵人的最大血量
        maxHP = GameDB.storm.enemybased.HP;
        currentHP = maxHP;
        Debug.Log($"暴風初始化完成，最大血量: {maxHP}");
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.A))
        {
            // 每次扣10點血量進行測試
            TakeDamage(10f);
            Debug.Log($"暴風受到10點傷害！當前血量: {currentHP}/{maxHP}");
        }
    ...
    public void TakeDamage(float damage)
    {
        ...
        // 計算實際傷害 (考慮防禦力)
        float actualDamage = Mathf.Max(1, damage - GameDB.storm.enemybased.Def);
        ...
        else if (animator != null && !animator.GetCurrentAnimatorStateInfo(0).IsName("dmg"))
        {
    ...
    private void Die()
    {
        ...
        StormMgr stormManager = GetComponentInParent<StormMgr>();
        if (stormManager != null)
        {
            stormManager.SetDead();
        }
